...
int main(){
    ...
    // structure to hold hints to be passed on to getaddrinfo
    struct addrinfo hints;
    //allocating necessary memory for the hints
    memset(&hints, 0, sizeof(hints));
    hints.ai_family = AF_INET;
    hints.ai_socktype = SOCK_DGRAM;
    hints.ai_flags = AI_PASSIVE;

    // to hold linked list of found addresses
    ...
    // checking successfull creation of the socket
    ...
    // to store the client's port/service number
    ...
    printf("Sender's address is %s and service (port) is %s.\n", address_buffer, service_buffer);
